...
let origPersons = [];
try {
  origPersons = $("キャスト・監督抽出").all();
} catch (e) {}
if (!origPersons.length) {
  try {
    origPersons = $("キャスト監督抽出コード").all();
  } catch (e) {}
}

const creditsNode = getNodeData("TMDb credits取得");
const shapedNode =
  getNodeData("映画データ整形Code") ||
  getNodeData("映画データ整形コード") ||
  {};
const movieCountry = String(shapedNode.country || "KR").toUpperCase();

// TMDB のレスポンス構造に柔軟対応
const castList = Array.isArray(creditsNode?.cast)
  ? creditsNode.cast
  : Array.isArray(creditsNode?.credits?.cast)
    ? creditsNode.credits.cast
    : [];
const crewList = Array.isArray(creditsNode?.crew)
  ? creditsNode.crew
  : Array.isArray(creditsNode?.credits?.crew)
    ? creditsNode.credits.crew
    : [];
const createdByList = Array.isArray(creditsNode?.created_by)
  ? creditsNode.created_by
  : Array.isArray(creditsNode?.credits?.created_by)
    ? creditsNode.credits.created_by
    : [];
...
const inferPersonCountry = (name, nameEn, defaultCountry) => {
  ...
  const isKoreanName =
    /^(キム|パク|チョン|ソン|イ|チェ|カン|ハン|イ・|キム・|パク・|チョン・|ソン・|チェ・|カン・|ハン・|ユン|ユン・)/.test(
      n,
    ) || /[\uac00-\ud7af]/.test(ne);
  ...
  const isJapaneseName =
    ...
    !isKoreanName &&
    !/^[A-Za-z\s]+$/.test(n);
  if (isJapaneseName && (defaultCountry === "JP" || !defaultCountry))
    return "JP";
...
jaDirectors.forEach((name, idx) => {
  if (name) enNameMap[name] = enDirectors[idx] || null;
});
jaCast.forEach((name, idx) => {
  if (name) enNameMap[name] = enCast[idx] || null;
});
...
inputItems.forEach((item, idx) => {
  ...
  function extractActorQid(searchList) {
    ...
    const regex =
      /actor|actress|director|film|artist|singer|배우|영화|감독|演劇|映画|俳優|女優|監督/i;
    ...
    if (
      searchList.length === 1 &&
      !/disambiguation|동음이의/i.test(
        searchList[0].description ||
          searchList[0].display?.description?.value ||
          "",
      )
    ) {
  ...
  let qid = bindingObj.person?.value
    ? bindingObj.person.value.split("/").pop()
    : null;

  if (!qid) {
    let searchResults = [];
    try {
      searchResults = $("Wikidata人物検索").all()[idx]?.json?.search || [];
    } catch (e) {}
  ...
  const jaName =
    jaDirectors[idx] || jaCast[idx - jaDirectors.length] || orig.name || "";
  const searchName =
    jaName || orig.name || orig.search_key || item.json?.name || "";
  if (!searchName || seenNames.has(searchName)) return;
  seenNames.add(searchName);

  const isDirector =
    orig.occupation === "監督" || jaDirectors.includes(searchName);
  let personObj = null;

  if (isDirector) {
    const targetEnName =
      enDirectors[idx] || orig.name_en || enNameMap[searchName] || "";
      ...
      personObj =
        crewList.find(
          (c) =>
            ...
            targetEnName &&
            (c.name?.toLowerCase() === targetEnName.toLowerCase() ||
              c.original_name?.toLowerCase() === targetEnName.toLowerCase()),
    ...
    const targetEnName =
      (castIndex >= 0 ? enCast[castIndex] : "") ||
      orig.name_en ||
      enNameMap[searchName] ||
      "";

    if (castList.length > 0) {
      personObj = castList.find(
        (c) =>
          (targetEnName &&
            (c.name?.toLowerCase() === targetEnName.toLowerCase() ||
              c.original_name?.toLowerCase() === targetEnName.toLowerCase())) ||
          ...
          (c.original_name &&
            (c.original_name === searchName ||
              c.original_name.includes(searchName))),
      );
  ...
  const nameEn =
    (isDirector ? enDirectors[idx] : enCast[jaCast.indexOf(searchName)]) ||
    orig.name_en ||
    enNameMap[searchName] ||
    personObj?.original_name ||
    personObj?.name ||
    null;
  const tmdbImg = personObj?.profile_path
  ...
  if (!genderVal) {
    const wikiGender =
      bindingObj?.genderLabel?.value || bindingObj?.gender?.value || "";
