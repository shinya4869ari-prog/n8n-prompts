...
5. **アーティスト名・曲名の表記ルール（日本・韓国以外は英語/アルファベット表記統一）**:
   - **日本・韓国**: 読者に馴染みがあるため「現地語（日本語・ハングル）」と「英語表記」の両方（または併記）を取得・出力してください。
   - **日本・韓国以外の全世界（例: ブータン、タイ、アラブ諸国、ギリシャ等）**: 一般の読者が読めない現地文字（ゾンカ文字、タイ文字、アラビア文字、ギリシャ文字等）は含めず、**「英語表記・アルファベット表記（英字名）」に統一** して出力してください（`artist_name` と `artist_name_en` に共に読みやすい英字表記をセット）。
