// Hangul to Katakana deterministic phonetic transliterator
function hangulToKatakana(hangul) {
  if (!hangul || typeof hangul !== 'string') return hangul;
  
  const CHO = ['ㄱ', 'ㄲ', 'ㄴ', 'ㄷ', 'ㄸ', 'ㄹ', 'ㅁ', 'ㅂ', 'ㅃ', 'ㅅ', 'ㅆ', 'ㅇ', 'ㅈ', 'ㅉ', 'ㅊ', 'ㅋ', 'ㅌ', 'ㅍ', 'ㅎ'];
  const JUNG = ['ㅏ', 'ㅐ', 'ㅑ', 'ㅒ', 'ㅓ', 'ㅔ', 'ㅕ', 'ㅖ', 'ㅗ', 'ㅘ', 'ㅙ', 'ㅚ', 'ㅛ', 'ㅜ', 'ㅝ', 'ㅞ', 'ㅟ', 'ㅠ', 'ㅡ', 'ㅢ', 'ㅣ'];
  const JONG = ['', 'ㄱ', 'ㄲ', 'ㄳ', 'ㄴ', 'ㄵ', 'ㄶ', 'ㄷ', 'ㄹ', 'ㄺ', 'ㄻ', 'ㄼ', 'ㄽ', 'ㄾ', 'ㄿ', 'ㅀ', 'ㅁ', 'ㅂ', 'ㅄ', 'ㅅ', 'ㅆ', 'ㅇ', 'ㅈ', 'ㅊ', 'ㅋ', 'ㅌ', 'ㅍ', 'ㅎ'];

  // Famous family names mapping for natural Japanese Katakana
  const surnameMap = {
    ...
    '채': 'チェ', '원': 'ウォン', '천': 'チョン', '방': 'パン', '공': 'コン', '강': 'カン', '현': 'ヒョン',
  ...
  const syllableMap = {
    ...
    '게': 'ゲ', '네': 'ネ', '데': 'デ', '레': 'レ', '메': 'メ', '베': 'ベ', '세': 'セ', '에': 'エ', '제': 'ジェ', '체': 'チェ', '케': 'ケ', '테': 'テ', '페': 'ペ', '헤': 'ヘ',
    ...
    '교': 'ギョ', '규': 'ギュ', '효': 'ヒョ', '표': 'ピョ', '묘': 'ミョ', '료': 'リョ', '쇼': 'ショ', '조': 'チョ',
    ...
    '열': 'ヨル', '철': 'チョル', '철': 'チョル', '일': 'イル', '필': 'ピル', '길': 'キル', '달': 'ダル', '팔': 'パル', '환': 'ファン', '관': 'クァン', '완': 'ウォン'
  };

  const words = hangul.split(/([,、\s/]+)/);
  return words.map(w => {
    if (/^[,、\s/]+$/.test(w)) return w;
    if (!/[\uac00-\ud7af]/.test(w)) return w;

    // Check if whole word is mapped
    let kana = '';
    const chars = Array.from(w);
    
    // Check first char as surname
    if (chars.length >= 2 && surnameMap[chars[0]]) {
      const surname = surnameMap[chars[0]];
      const given = chars.slice(1).map(c => syllableMap[c] || c).join('');
      return `${surname}・${given}`;
    }

    return chars.map(c => syllableMap[c] || c).join('');
  }).join('');
}

const testList = '박규태, 신세경, 정유진, 이신기, 강하경, 이준영, 김의성, 장현성, 주보비, 우정원, 김은우, 조은형, 채수하, 박한솔, 오은재, 김송은';
console.log('Original:');
console.log(testList);
console.log('\nConverted Katakana:');
console.log(hangulToKatakana(testList));
