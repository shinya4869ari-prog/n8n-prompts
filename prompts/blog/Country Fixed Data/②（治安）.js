const researcherNode = $('固定データ Researcher').first().json;

// output, text, content, message.content など Gemini/AI ノードのあらゆる出力形式に対応
function extractRawText(node) {
  if (!node) return "";
  if (typeof node === 'string') return node;
  if (node["治安・社会指標"] || node["物価"]) return node; // 既にパース済みのオブジェクト
...
let data;
if (typeof rawCandidate === 'object' && rawCandidate !== null && (rawCandidate["治安・社会指標"] || rawCandidate["物価"])) {
  data = rawCandidate;
} else {
...
const getPrisonData = (index, field) => {
  ...
  if (field === "年") return entry["年"] || "";
  if (field === "数") return entry["総収容者数"] || "";
  return "";
};
...
return [{
  json: {
    ...
    "刑務所稼働率": wpb["刑務所稼働率"]?.["値"] || "",
    "刑務所稼働率_年": wpb["刑務所稼働率"]?.["年"] || "",
    "刑務所稼働率_出典": wpb["刑務所稼働率"]?.["出典"] || "",
    "刑務所総収容者数": wpb["刑務所総収容者数"]?.["値"] || "",
    "刑務所総収容者数_年": wpb["刑務所総収容者数"]?.["年"] || "",
    "刑務所総収容者数_出典": wpb["刑務所総収容者数"]?.["出典"] || "",
