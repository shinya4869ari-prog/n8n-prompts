...
async function upsertCleanMovies() {
  ...
  for (const m of movies) {
    if ((m.director && HANGUL_REGEX.test(m.director)) || (m.cast && HANGUL_REGEX.test(m.cast))) {
      m.director = toKatakanaIfHangul(m.director);
      m.cast = toKatakanaIfHangul(m.cast);

      console.log(`Upserting Movie "${m.title}"...`);
      const postRes = await fetch(`${SUPABASE_URL}/rest/v1/Movies?on_conflict=title`, {
        method: 'POST',
        headers: {
          'apikey': SUPABASE_KEY,
