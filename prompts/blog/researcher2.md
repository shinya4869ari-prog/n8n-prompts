...
- 各事象：年号・事象名・概要（2〜3文）
- 検索クエリ：「{{ $json.country }} modern history major events 20th 21st century」
- 深刻な事件（虐殺・紛争・クーデター等）は必ず含めること
- Deep Dive候補として最も重要な事象を1つ選定し記載すること
...
- 最新6ヶ月の政治・経済・社会の動向
  - 検索クエリ：「{{ $json.country }} latest news {{ $now.toFormat('yyyy') }}」
  - 検索クエリ：「{{ $json.country }} economy politics social {{ $now.toFormat('yyyy') }}」
...
- 検索クエリ：「{{ $json.country }} documentary film imdb」
- 検索クエリ：「{{ $json.country }} 映画 実話 Wikipedia」
- 検索クエリ：「{{ $json.country }} war genocide human rights film」（紛争・虐殺がある国のみ）
...
- 検索クエリ：「{{ $json.country }} highest grossing films all time domestic box office」
- 検索クエリ：「{{ $json.country }} 映画 歴代 興行収入 ランキング」
- 対象国の純粋な国内映画に限定
- 各作品について収集：
  - 順位（1〜10）
