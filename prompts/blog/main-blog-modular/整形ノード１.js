const r1Raw = $('01_basic_institution').first().json;
const r2Raw = $('02_history_trendser2').first().json; // Perplexityノードの出力
...
const parseOutput = (node, nodeName) => {
  try {
    ...
    // ★最優先：もし入力自体が二重エスケープ等の「文字列」なら、まずデコードして本来のオブジェクトや文字列にする
...
const japanFixed = {
  ...
  物価: {
    ...
    '光熱費（月額・85㎡）': { '値（円）': jBukka['光熱費_円換算'] ? `${jBukka['光熱費_円換算']}円` : 'データなし', 出典: jBukka['光熱費_出典'] },
