...
let auditResult = {};
const rawText =
  inputJson.content?.parts?.[0]?.text ||
  inputJson.parts?.[0]?.text ||
  inputJson.output ||
  inputJson.text ||
  (typeof inputJson.message?.content === "string"
...
const isValid =
  auditResult.is_valid !== false &&
  (auditResult.confidence_score === undefined ||
    auditResult.confidence_score >= 70);
...
if (origTracks.length === 0) {
  try {
    origTracks = $("OST劇中歌取得整形Code")
      .all()
      .map((item) => item.json)
      .filter((t) => t && t.track_id && t.has_tracks !== false);
  } catch (e) {}
}
...
const cleanCastString = (castStr) => {
  ...
  const excludeKeys = dirNames
    .map((n) => n.replace(/[\s・\.\-]/g, "").toLowerCase())
    .filter(Boolean);
  const list = castStr
    .split(/[,、/]+/)
    .map((s) => cleanNameNoise(s))
    .filter(Boolean);
  const seen = new Set();
  const deduped = [];
  list.forEach((name) => {
...
if (
  origMovie.title &&
  origMovie.title.trim() &&
  !/^[A-Za-z0-9\s:,'"-]+$/.test(origMovie.title)
) {
...
(origPersons.length > 0 ? origPersons : aiPersons).forEach((origP) => {
  ...
  const matchedAi = aiPersons.find(
    (ap) =>
      ((ap.wikidata_id || ap.qid) &&
        (origP.wikidata_id || origP.qid) &&
        (ap.wikidata_id || ap.qid) === (origP.wikidata_id || origP.qid)) ||
      (ap.name &&
        rawName &&
        (cleanNameNoise(ap.name) === rawName ||
          cleanNameNoise(ap.name).includes(rawName) ||
          rawName.includes(cleanNameNoise(ap.name)))),
  );

  const wId =
    origP.wikidata_id ||
    origP.qid ||
    matchedAi?.wikidata_id ||
    matchedAi?.qid ||
    null;
  const pNameEn =
    hasHangul(origP.name_en) && !hasHangul(matchedAi?.name_en)
      ? origP.name_en
  ...
  const pObj = {
    ...
    profile_url:
      origP.profile_url || matchedAi?.profile_url || origP.image_url || null,
...
if (rawAiTracks.length > 0) {
  approvedTracks = rawAiTracks
    .filter((t) => {
      if (!t || (!t.track_id && !t.track_name)) return false;
      if (t.status && String(t.status).toUpperCase() === "REJECTED")
        return false;
      return true;
    })
    .map((t) => {
      const matchedOrig = origTracks.find(
        (ot) =>
          String(ot.track_id) === String(t.track_id) ||
          (ot.track_name &&
            t.track_name &&
            ot.track_name.includes(t.track_name)),
      );

      // 韓国作品の場合、ハングル表記を最優先保護
      const artistEn =
        hasHangul(matchedOrig?.artist_name_en) && !hasHangul(t.artist_name_en)
      ...
      const trackEn =
        hasHangul(matchedOrig?.track_name_en) && !hasHangul(t.track_name_en)
      ...
      return {
        ...
        release_year: String(
          t.release_year || matchedOrig?.release_year || movie.year || "",
        ),
        preview_url: t.preview_url || matchedOrig?.preview_url || "",
        itunes_url: t.itunes_url || matchedOrig?.itunes_url || "",
        album_cover:
          t.album_cover || matchedOrig?.album_cover || movie.poster_url || "",
        description:
          t.description ||
          matchedOrig?.description ||
          `映画・ドラマ『${movie.title}』公式挿入歌。`,
...
if (!Array.isArray(auditResult.tracks) && origTracks.length > 0) {
  approvedTracks = origTracks
    .filter((ot) => {
      ...
      if (
        /dalpalan|달파란|タルパラン|조영욱|チョ・ヨンウク|limeland/i.test(
          ot.artist_name || "",
        )
      )
        return false;
