{
  "title": "{{ (() => { try { const node = $('キャスト・監督翻訳AI').item?.json; const raw = node?.text || (Array.isArray(node?.content) ? node.content[0]?.text : node?.content) || ''; const parsed = JSON.parse(raw.replace(/```json/gi, '').replace(/```/g, '').trim()); return parsed.title || $('映画データ整形コード').item.json.title || ''; } catch(e) { return $('映画データ整形コード').item.json.title || ''; } })() }}",
  ...
  "overview": "{{ (() => { try { const aiText = $('gemini_movie_db').item?.json?.text || $('claude_movie_db').item?.json?.text || (Array.isArray($('gemini_movie_db').item?.json?.content) ? $('gemini_movie_db').item?.json?.content[0]?.text : null); if (aiText) { const isRefusal = aiText.includes('申し訳あ') || aiText.includes('情報がございません') || aiText.includes('追加情報') || aiText.includes('作成できません') || aiText.includes('限定的') || aiText.trim().length < 30; if (!isRefusal) return aiText.replace(/\\/g, '\\\\').replace(/"/g, '\\"').replace(/\n/g, '\\n').replace(/\r/g, '\\r'); } return $('映画データ整形コード').item.json.overview || ''; } catch(e) { return $('映画データ整形コード').item.json.overview || ''; } })() }}",
  ...
  "director_en": "{{ (() => { try { const node = $('キャスト・監督翻訳AI').item?.json; const raw = node?.text || (Array.isArray(node?.content) ? node.content[0]?.text : node?.content) || ''; const parsed = JSON.parse(raw.replace(/```json/gi, '').replace(/```/g, '').trim()); return parsed.director_en || $('映画データ整形コード').item.json.director_en || ''; } catch(e) { return $('映画データ整形コード').item.json.director_en || ''; } })() }}",
  "cast_en": "{{ (() => { try { const node = $('キャスト・監督翻訳AI').item?.json; const raw = node?.text || (Array.isArray(node?.content) ? node.content[0]?.text : node?.content) || ''; const parsed = JSON.parse(raw.replace(/```json/gi, '').replace(/```/g, '').trim()); return parsed.cast_en || $('映画データ整形コード').item.json.cast_en || ''; } catch(e) { return $('映画データ整形コード').item.json.cast_en || ''; } })() }}"
}
