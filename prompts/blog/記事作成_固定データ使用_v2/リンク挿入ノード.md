...
// ② 入力データの取得
const mainArticle = $input.first().json.article ?? '';
const country = $input.first().json.country ?? '不明';

// ③ リンク挿入処理
let processedArticle = mainArticle;
...
// ⑦ 分離置換
let tokens = processedArticle.split(/(<[^>]+>)/g).filter(p => p).map(p => ({
