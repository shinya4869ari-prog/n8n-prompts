...
- 各事象：年号・事象名・概要（2〜3文）
- 検索クエリ：「{{ $('On form submission').first().json.countryEn }} modern history major events 20th 21st century」
...
- 最新6ヶ月の政治・経済・社会の動向
  - 検索クエリ：「{{ $('On form submission').first().json.countryEn }} latest news {{ $now.toFormat('yyyy') }}」
  - 検索クエリ：「{{ $('On form submission').first().json.countryEn }} economy politics social {{ $now.toFormat('yyyy') }}」
...
- 検索クエリ：「{{ $('On form submission').first().json.countryEn }} documentary film imdb」
- 検索クエリ：「{{ $('On form submission').first().json.countryEn }} true story film Wikipedia」
- 検索クエリ：「{{ $('On form submission').first().json.countryEn }} war genocide human rights film」（紛争・虐殺がある国のみ）
...
- 検索クエリ：「{{ $('On form submission').first().json.countryEn }} highest grossing films all time domestic box office」
- 検索クエリ：「{{ $('On form submission').first().json.countryEn }} top 10 films box office ranking all time」
- 対象国の純粋な国内映画に限定
- 各作品について収集：
  - 順位（1〜10）
