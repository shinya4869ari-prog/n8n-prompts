...
- **【重要：想像・補完の絶対禁止ルール（ハルシネーション対策）】**：
  - 「犯人名」や「判決」が、検索結果（Wikipedia等の記事）から実名・事実として確実に確認できない場合は、絶対に推測で書いてはいけません。
  - 確実な裏付けがない項目は、適当に埋めず、必ず **`「不明」`** または **`「未確認」`** と記述してください。（例：犯人が逃亡中で逮捕されていない場合は、判決を「逮捕（判決不明）」ではなく「逃亡中・未逮捕」とし、犯人名がわからない場合は「不明」としてください）。
- **【重要：検証用URLの出力義務】**：
...
- **【検索ツールの使用義務と正確性】**
  ...
  - 「発生年」は、検索結果から確認できる実際の発生（または判決）の西暦4桁を正確に記載してください。
