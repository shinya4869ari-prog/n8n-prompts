...
const prevNodes = ['検索結果まとめ記事', 'Google Gemini'];
...
// 4. トークン未取得時の推計（まとめ記事入力用: 約4,500 tok）
...
const costSummary = `💰 まとめ記事コスト: ${costJpyFormatted} (${costUsdFormatted}) [${detectedModel} | 入力: ${promptTokens.toLocaleString()} tok / 出力: ${completionTokens.toLocaleString()} tok]`;

// 6. 返却（後続の即時保存ノードへそのまま渡せるよう元のitemをマージ）
return [{
  json: {
