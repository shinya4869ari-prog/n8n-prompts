...
- 日本語フィールド（title, director, cast, overview, genres）の文字表記絶対禁止ルール:
  1. 日本語フィールド内にハングル文字（가-힣）が1文字でも残存・混入することを【絶対禁止】とします。必ず完全に純粋な日本語（カタカナ/ひらがな/漢字）へ変換し切ってください（例: "キム・ビ비" ❌ ➔ "キム・ビビ" ⭕️、"ドキュ멘タリー" ❌ ➔ "ドキュメンタリー" ⭕️）。
  2. カタカナ名前の中黒（・）の直前に、不要なハイフン（-）を結合させることを【絶対禁止】とします（例: "イ-・イダム" ❌ ➔ "イ・イダム" ⭕️、"チン-・ソジン" ❌ ➔ "チン・ソジン" ⭕️）。
