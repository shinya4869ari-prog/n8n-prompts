...
const jPrison = Array.from({length: 10}, (_, i) => ({
  年: jChiAn[`収容推移${i+1}_年`],
  総収容者数: jChiAn[`収容推移${i+1}_総収容者数`]
})).filter(r => r.年);

const jDeath = Array.from({length: 10}, (_, i) => ({
  順位: `${i+1}位`,
  死因: jChiAn[`死因${i+1}位`]
}));
...
const targetFixed = {
  ...
  刑務所推移: Array.from({length: 10}, (_, i) => ({
    年: chiAn[`収容推移${i+1}_年`],
    総収容者数: chiAn[`収容推移${i+1}_総収容者数`]
  })).filter(r => r.年),
  死因トップ10: Array.from({length: 10}, (_, i) => chiAn[`死因${i+1}位`]),
  物価: {
    通貨コード: bukka['通貨コード'],
    為替レート: bukka['為替レート'],
    為替取得日: bukka['為替取得日'],
