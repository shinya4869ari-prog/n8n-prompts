...
入力データに含まれる title, year, country, director, overview などの基本情報と、tmdb_id, poster_url, trailer_url などのID・URL情報を照合してください。
1. **安易な誤判定の禁止**: 入力された tmdb_id, poster_url, trailer_url が該当映画の正当なものである場合は絶対に削除・クリアしないでください。特に最新の小規模映画やアジア映画のIDを勝手に別映画と決めつけないでください。
2. 明らかな他作品の混入のみクリア: 検索・検証の結果、明らかにタイトルや制作国と全く異なる別映画（例: ドキュメンタリー作品に対して全く無関係なハリウッドアクション映画のURLが誤って混入している場合など）であることが確実な場合のみ、該当の tmdb_id を null に、poster_url や trailer_url を "" (空文字) にクリアしてください。
3. あらすじ（overview / overview_en）の照合: タイトルや国と矛盾する全く別映画のあらすじが入っている場合は、正解の内容に修正してください。また日本語と英語で内容が食い違っている場合は内容を統一・詳細化してください。
4. audit_status: 正常なら "OK"、明確な誤データ消去・修正時のみ "CORRECTED_MISMATCH: [明確な理由]" と記載してください。
