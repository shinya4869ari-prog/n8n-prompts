...
<h2 id="section-3" style="${h2Style}"><span style="background:#00bcd4;color:#fff;border-radius:6px;padding:2px 10px;font-size:13px;font-weight:500;">③</span> 治安と平和の衡量</h2>
<table style="${tableStyle}">
  <thead>
    <tr>
      <th style="${thStyle('35%')}">治安・社会指標</th>
      <th style="${thStyle('32%')}">${countryLabel}</th>
      <th style="${thStyle('33%')}">${japanLabel}</th>
    </tr>
  </thead>
  <tbody>
`;
...
tableRows.forEach((row, ri) => {
  ...
  html += `    <tr style="${bg}">
      <td style="${tdBoldStyle}">${row.name}</td>
...
if (kikenLevel >= 1) {
  ...
  html += `<p style="color:#d32f2f;font-weight:bold;background:#fff3f3;padding:10px;border-radius:8px;">${msg}</p>\n`;
}
