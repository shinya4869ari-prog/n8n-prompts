...
const wb補完 = (researcherVal, researcherYear, wbKey) => {
  const wbData = wb[wbKey];
  if (wbData && wbData.値 !== null && wbData.値 !== undefined) {
    const wbYear = parseInt(wbData.年) || 0;
    const rYear = parseInt(researcherYear) || 0;
    if (wbYear >= rYear) return String(wbData.値);
  }
  return researcherVal || "";
};
const wb補完年 = (researcherVal, researcherYear, wbKey) => {
  const wbData = wb[wbKey];
  if (wbData && wbData.年) {
    const wbYear = parseInt(wbData.年) || 0;
    const rYear = parseInt(researcherYear) || 0;
    if (wbYear >= rYear) return String(wbData.年);
  }
  return researcherVal || "";
};
const wb補完出典 = (researcherVal, researcherYear, wbKey) => {
  const wbData = wb[wbKey];
  if (wbData && wbData.出典) {
    const wbYear = parseInt(wbData.年) || 0;
    const rYear = parseInt(researcherYear) || 0;
    if (wbYear >= rYear) return wbData.出典;
  }
  return researcherVal || "";
};
...
return [{
  json: {
    ...
    // --- 既存項目（WB補完あり）---
    "殺人率": wb補完(t["殺人率"]?.["値"], t["殺人率"]?.["年"], "殺人率"),
    "殺人率_年": wb補完年(t["殺人率"]?.["年"], t["殺人率"]?.["年"], "殺人率"),
    "殺人率_出典": wb補完出典(t["殺人率"]?.["出典"], t["殺人率"]?.["年"], "殺人率"),

    "交通事故死亡率": wb補完(t["交通事故死亡率"]?.["値"], t["交通事故死亡率"]?.["年"], "交通事故死亡率"),
    "交通事故死亡率_年": wb補完年(t["交通事故死亡率"]?.["年"], t["交通事故死亡率"]?.["年"], "交通事故死亡率"),
    "交通事故死亡率_出典": wb補完出典(t["交通事故死亡率"]?.["出典"], t["交通事故死亡率"]?.["年"], "交通事故死亡率"),

    "自殺率": wb補完(t["自殺率"]?.["値"], t["自殺率"]?.["年"], "自殺率"),
    "自殺率_年": wb補完年(t["自殺率"]?.["年"], t["自殺率"]?.["年"], "自殺率"),
    "自殺率_出典": wb補完出典(t["自殺率"]?.["出典"], t["自殺率"]?.["年"], "自殺率"),

    "失業率": wb補完(t["失業率"]?.["値"], t["失業率"]?.["年"], "失業率"),
    "失業率_年": wb補完年(t["失業率"]?.["年"], t["失業率"]?.["年"], "失業率"),
    "失業率_出典": wb補完出典(t["失業率"]?.["出典"], t["失業率"]?.["年"], "失業率"),

    "貧困率": wb補完(t["貧困率"]?.["値"], t["貧困率"]?.["年"], "貧困率"),
    "貧困率_年": wb補完年(t["貧困率"]?.["年"], t["貧困率"]?.["年"], "貧困率"),
    "貧困率_出典": wb補完出典(t["貧困率"]?.["出典"], t["貧困率"]?.["年"], "貧困率"),

    "ジニ係数": wb補完(t["ジニ係数"]?.["値"], t["ジニ係数"]?.["年"], "ジニ係数"),
    "ジニ係数_年": wb補完年(t["ジニ係数"]?.["年"], t["ジニ係数"]?.["年"], "ジニ係数"),
    "ジニ係数_出典": wb補完出典(t["ジニ係数"]?.["出典"], t["ジニ係数"]?.["年"], "ジニ係数"),
    ...
    "女性労働参加率": wb補完(w["女性労働参加率"]?.["値"], w["女性労働参加率"]?.["年"], "女性労働参加率"),
    "女性労働参加率_年": wb補完年(w["女性労働参加率"]?.["年"], w["女性労働参加率"]?.["年"], "女性労働参加率"),
    "女性労働参加率_出典": wb補完出典(w["女性労働参加率"]?.["出典"], w["女性労働参加率"]?.["年"], "女性労働参加率"),

    "女性議員比率": wb補完(w["女性議員比率"]?.["値"], w["女性議員比率"]?.["年"], "女性議員比率"),
    "女性議員比率_年": wb補完年(w["女性議員比率"]?.["年"], w["女性議員比率"]?.["年"], "女性議員比率"),
    "女性議員比率_出典": wb補完出典(w["女性議員比率"]?.["出典"], w["女性議員比率"]?.["年"], "女性議員比率"),

    "児童労働率": wb補完(w["児童労働率"]?.["値"], w["児童労働率"]?.["年"], "児童労働率"),
    "児童労働率_年": wb補完年(w["児童労働率"]?.["年"], w["児童労働率"]?.["年"], "児童労働率"),
    "児童労働率_出典": wb補完出典(w["児童労働率"]?.["出典"], w["児童労働率"]?.["年"], "児童労働率"),
  }
}];