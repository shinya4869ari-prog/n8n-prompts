const TARGET_NODES = [
  ...
  "Geminiスクショ解析",
  "gemini_movie_db",
  "gemini_movie_db_prompt",
  "Gemini人物クレンジング",
  "映画_人物_OST_総合AI検証プロンプト",
  "キャスト翻訳",
];
...
const PRICING = {
  "gemini-3.8-flash": { in: 0.1, out: 0.4 },
  "gemini-3.1-pro": { in: 1.25, out: 5.0 },
  "gpt-5": { in: 2.5, out: 10.0 },
  o3: { in: 5.0, out: 20.0 },
  "o4-mini": { in: 0.15, out: 0.6 },
  "claude-sonnet-4": { in: 3.0, out: 15.0 },
  sonar: { in: 1.0, out: 1.0, fee: 0.005 },
  "sonar-pro": { in: 3.0, out: 15.0, fee: 0.005 },
};
...
function resolveModel(str) {
  ...
  if (s.includes("pro"))
    return { name: "gemini-3.1-pro", r: PRICING["gemini-3.1-pro"] };
  if (s.includes("o3")) return { name: "o3", r: PRICING["o3"] };
  if (s.includes("gpt-5") || s.includes("gpt-4o"))
    return { name: "gpt-5", r: PRICING["gpt-5"] };
  if (s.includes("claude"))
    return { name: "claude-sonnet-4", r: PRICING["claude-sonnet-4"] };
  if (s.includes("sonar-pro"))
    return { name: "sonar-pro", r: PRICING["sonar-pro"] };
...
const nodeDetails = [],
  flatList = {},
  usedModels = new Set();
const scanList = TARGET_NODES.length
  ? TARGET_NODES
  : ["Message a model", "AI Agent", "Gemini", "OpenAI", "writer"];
...
for (const name of scanList) {
  try {
    ...
    for (const item of items) {
      ...
      const u =
        d.usageMetadata ||
        d.response?.usageMetadata ||
        d.tokenUsageEstimate ||
        {};
      ...
      if (p || c) {
        ...
        if (txt) {
          pTok += 1500;
          cTok += Math.round(txt.length * 0.95);
        }
      }
    ...
    totalCompl += cTok;
    totalTok += pTok + cTok;
    searches += reqSearches;
    flatList["📍 " + name] =
      formatJpy(jpy) +
      " (" +
      mName +
      " / " +
      (pTok + cTok).toLocaleString() +
      " tok)";
    nodeDetails.push({
      node: name,
      model: mName,
      ...
      completion: cTok,
      total: pTok + cTok,
      cost_jpy: formatJpy(jpy),
    });
  } catch (_) {}
}

const primaryModel = usedModels.size
  ? Array.from(usedModels).join(", ")
  : "gemini-3.8-flash";
...
return [
  {
    json: {
      "💰【総AIコスト】":
        formatJpy(totalJpy) + " ($" + totalUsd.toFixed(4) + ")",
      "────────── 費用明細 ──────────": "───────────────────────────────",
      ...flatList,
      generation_cost: {
        model: primaryModel,
