...
async function fixRemainingMovies() {
  const targetIds = [1387140, 1451344, 639988, 290098, 982843];
  
  for (const tid of targetIds) {
    const res = await fetch(`${SUPABASE_URL}/rest/v1/Movies?tmdb_id=eq.${tid}`, {
    ...
    console.log(`Fixing Movie ${tid} (${m.title}):`);
    console.log(`  Dir: ${m.director} ➡️ ${newDir}`);
    console.log(`  Cast: ${m.cast?.slice(0, 50)}... ➡️ ${newCast?.slice(0, 50)}...`);

    const patchRes = await fetch(`${SUPABASE_URL}/rest/v1/Movies?tmdb_id=eq.${tid}`, {
      method: 'PATCH',
      headers: {
        'apikey': SUPABASE_KEY,
