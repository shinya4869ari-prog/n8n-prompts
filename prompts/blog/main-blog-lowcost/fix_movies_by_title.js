...
const SURNAMES_MAP = {
  ...
  '남': 'ナム', '심': 'シム', 'ノ': 'ノ', '하': 'ハ', '곽': 'クァク', '성': 'ソン', '차': 'チャ',
...
async function fixAllMoviesByOrigin() {
  const resM = await fetch(`${SUPABASE_URL}/rest/v1/Movies?select=*&limit=1000`, {
    headers: { 'apikey': SUPABASE_KEY, 'Authorization': `Bearer ${SUPABASE_KEY}` }
  });
  const movies = await resM.json();

  for (const m of movies) {
    if ((m.director && HANGUL_REGEX.test(m.director)) || (m.cast && HANGUL_REGEX.test(m.cast))) {
      const newDir = toKatakanaIfHangul(m.director);
      const newCast = toKatakanaIfHangul(m.cast);

      console.log(`Fixing Movie "${m.title}" / "${m.origin_title}"...`);
      const patchRes = await fetch(`${SUPABASE_URL}/rest/v1/Movies?title=eq.${encodeURIComponent(m.title)}`, {
        method: 'PATCH',
        headers: {
          'apikey': SUPABASE_KEY,
      ...
      console.log(`Updated result for "${m.title}":`, updated.length > 0 ? "SUCCESS" : "FAILED");
    }
  }
}

fixAllMoviesByOrigin();
