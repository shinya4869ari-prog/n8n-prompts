...
5. **現地語表記と英語表記の併記ルール**:
   - 映画の「タイトル」と「原題」と同様に、アーティスト名および曲名は **「現地語（ハングル・現地文字）」と「英語表記」の両方** を取得・補正してください。
   - `artist_name`: 現地語表記（例: `다이나믹 듀오 & 디제이 프리미어`）※カタカナ等の文字化けは正しく補正
   - `artist_name_en`: 英語・原語表記（例: `Dynamic Duo & DJ Premier`）
   - `track_name`: 現地語表記
   - `track_name_en`: 英語・原語表記
