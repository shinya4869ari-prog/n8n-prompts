    ...
    const bukkaRows = bukkaData.map(d => {
      ...
      if (d['項目'].includes('Netflix')) {
        ...
        if (netflixItem && netflixItem.現地通貨 && String(netflixItem.現地通貨) !== 'データなし') {
          ...
          if (currencySymbol && !localVal.startsWith(currencySymbol) && !localVal.startsWith('$') && !localVal.startsWith('€') && !localVal.startsWith('£')) {
...
<div style="background:${bg};border:1px solid #eef2f5;border-radius:12px;padding:18px 20px;margin:20px 0;box-shadow:0 4px 15px rgba(0,0,0,0.05);position:relative;overflow:hidden;">
  ...
  try {
    ...
    deepDiveArticle = extractDeepDiveText(inputJson.deepDiveArticle)
    ...
    if (!deepDiveArticle) {
      deepDiveArticle = extractDeepDiveText(sheetData?.data?.対象国データ_記事?.Deep_Dive)
    ...
    if (!deepDiveArticle) {
      try {
        ...
        for (const item of all) {
          const j = item.json || {};
          const found = extractDeepDiveText(j.deepDiveArticle)
                     || extractDeepDiveText(j.deep_dive)
                     || (j.article && j.article.includes('✦') ? extractDeepDiveText(j.article) : '');
          if (found) {
            deepDiveArticle = found;
            break;
          }
