...
5. **出典の記載方法**: 各項目の「出典」には、純粋な機関・組織名（例：「UNODC」「警察庁」など）のみを記載してください。「10万人あたり」「Rape rate per 100,000」などの算出基準や単位、その他の補足情報は含めないこと。
...
### 刑務所データ（稼働率・総数・推移）の絶対ルール
**出典は必ず「World Prison Brief」を使用すること。世界銀行、UNODC、各国統計局などのデータは一切使用禁止。**
1. **刑務所稼働率（Occupancy level）**: 定員に対する収容割合を **「〇〇%」** の形式で収集すること。
...
- 最新年、出典（各国警察統計など）を明記。
- 具体的名称（例：窃盗、暴行、強盗、詐欺、麻薬犯罪）で、必ず日本語のみでリストアップ（英語併記は不可）。
...
- 性的暴行（Sexual violence/Rape）の届出率（10万人あたり）
  - **※重要**: 出典には「10万人あたり」や「per 100,000」などの表記は一切含めず、純粋な組織名（例: UNODC, 警察庁）のみを記載すること。
...
- **※絶対ルール**: 現地通貨建ての価格を出力する際は、**必ず価格の前に通貨シンボル（例: `$`, `€`, `£`, `₩` など）を付与して記載すること**（例: `$14.99`, `€12.99`, `₩13,500`）。また、「通貨コード」にはその通貨コード（例：KRW, EUR）を記載すること。
- Netflixが現地通貨非対応の国（例：アフガニスタン等）はUSD建ての価格をシンボル付きで出力し（例: `$7.99`）、「通貨コード」に「USD」を記載すること。
...
## 6. 貿易
優先出典：OEC (Observatory of Economic Complexity) → TrendEconomy → UN Comtrade → WITS → World Bank → Eurostat → 各国統計局 → IMF

提供された検索結果などを元に、以下のデータを調査・抽出してください。
- 主要輸出項目（1〜10位）および主要輸入項目（1〜10位）: 品目名を日本語に翻訳してリストアップ。
- 主要貿易相手国: 国名を日本語に翻訳し、シェア（%付き。不明な場合のみ空欄可）、および出典名と調査年を明記。

※注意：
- 英語の品目名や国名は必ず日本語に翻訳して出力形式に当てはめること。
- データがどうしても存在しない項目は「欠測」とすること。
---
