...
- **【検索ツールの使用義務と正確性】**
  - **必ず現在接続されている検索ツール（PerplexityやWeb Search等）を使用して、対象国の実際の重大犯罪事件をウェブ検索し、そこから得られた事実のみを収集してください。** 自身の学習知識による推測や空想で事件を出力することは厳禁です。
  - 収集した事件の「出典」には、検索結果に表示されている実際のメディアやソース（例：BBC News, Reuters, Wikipedia, The New York Timesなど）を正確に記載してください。事件と無関係な国の組織（例：ニュージーランドの事件ではないのに「Courts of New Zealand」と書くなど）を出典に捏造することは厳禁です。
  - 「発生年」は、検索結果から確認できる実際の発生（または判決）の西暦4桁を正確に記載してください。適当な年号（実際は2025年なのに2024年とするなど）の出力は禁止します。
